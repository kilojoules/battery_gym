...
In every (market, year) the diagnostic returns ``invariance survives'': the optimal capacity $b_E^*$ is identical across deterministic and stochastic dispatch policies, even though stochastic dispatch yields $9$--$34\%$ realized-NPV uplift at the optimum. The theoretical sketch predicted that multi-day spectral content should break invariance. It does not. We surface the falsification explicitly (\S\ref{sec:results}), discuss candidate mechanisms (\S\ref{sec:discussion}), and note that the diagnostic survives the falsification: the proposition's job was to motivate the test, not to determine its outcome.
